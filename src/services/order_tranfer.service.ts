...
export async function findAllOrderTranfer(): Promise<OrderTranfer[]> {
  try {
    const result = await endpoint.get<OrderTranfer[]>(
      enviromentDev.orderTranfer,
    );
    return result.data;
  } catch (error) {
    throw error;
...
export async function findOrderTranferByOrderId(order_tranfer_id: string) {
  try {
    return endpoint.get<OrderTranfer>(
      `${enviromentDev.orderTranfer}/${order_tranfer_id}`,
    );
  } catch (error) {
    throw error;
  }
...
export function CompletedOrderTranferByOrderId(order_tranfer_id: string) {
  try {
    return endpoint.put<OrderTranfer>(
      `${enviromentDev.orderTranfer}/close-order/${order_tranfer_id}`,
    );
  } catch (error) {
    throw error;
  }
...
export function editOrderTranferByOrderId(

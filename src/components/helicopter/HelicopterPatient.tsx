...
export default function HeliopterPatient({ patient, heliopter_id }: Props) {
    ...
    function handleAddMonitorInPatient() {
        setIsLoad(true)
        window.location.href = '/vehicle/' + heliopter_id + '/car/detail/add_monitor?patient_add_id=' + patient.id
    }
    return (
        <>
                ...
                <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                    ...
                    <Stack className='m-1' direction="row" spacing={1}>
                        <Chip
                            ...
                            onClick={() => {
                                setIsLoad(true)
                                window.location.href = `/vehicle?tranfrom=helicopter&car_id=${heliopter_id}&patient_id=${patient.id}`

...
const [maxLength, setMaxLenght] = useState<{
  ...
  async function fetchData() {
    ...
    const CHANNELS: ChannelInfo[] = new Array(channelCount).fill(0).map((_, i) => ({ name: `ECG-${i + 1}`, yMin: maxLength.delete, yMax: maxLength.add }));
    ...
    streamData();
    let chartR = document.getElementById('ecg');
  ...
  return (
    <>
      <div id='ecg' ref={chartRefEcg}></div>
    </>
  )
};

...
import { createHistory, findHistoryByPatientId } from '@/services/history.service';
...
export default function page() {
  ...
  const [historyFilter, setHistoryFilter] = React.useState<Historys[]>({} as Historys[])
  ...
  async function onChangFilterHistory(key: number) {
    ...
    switch (true) {
      ...
      case key === 1:
        return setHistoryFilter(history.filter(r => r.status.includes('Draft')))
      case key === 2:
        return setHistoryFilter(history.filter(r => r.status.includes('Completed')))
      case key === 3:
        return setHistoryFilter(history.filter(r => r.status.includes('Cancel')))
      case key === 4:
        return setHistoryFilter(history.filter(r => r.status.includes('Close')))
    }
      ...
      <div className={historyCss.homePage}>
        <div className={historyCss.item}>
          ...
          <div className={historyCss.history_item}>
            {
              history.length > 0 ?
                historyFilter.map((r, i) =>
                  <HistoryItem value={r} key={i} />
                ) :
                null
            }
          </div>
          ...
          >
            ...
            >
              ...
              <Input
                ...
                disabled={created}
              />
              <Button sx={{ fontSize: '1.2rem', width: '100%', marginTop: '10px' }} onClick={onCreateHistory} loading={created}>Add History</Button>
              <div hidden={!created}>
                <Alert sx={{ marginTop: '10px' }} icon={<CheckIcon fontSize="inherit" />} severity="success">
                  Created History
                </Alert>
                <Box className={historyCss.buttonGroup}>
                  <Button className={historyCss.button} onClick={() => setHistoryFrom(false)} color='neutral'>Cancel</Button>
                  <Button className={historyCss.button} onClick={() => {
                    router.push('history/' + history_id)
                    setIsLoad(true)
                  }}
                    startDecorator={<AddCircleIcon />} color='success'>Add Exan</Button>
                </Box>
              </div>
            </Sheet>
          </Modal> :

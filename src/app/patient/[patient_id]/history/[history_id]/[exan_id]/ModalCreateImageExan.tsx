...
import { TModalImageExan, ModalImageExanContext, TModalCreate, OpenModalCreateImageContext } from './page';
...
export default function ModalCreateImageExan({params}: Props) {
    ...
    const { open, setOpen } = React.useContext<TModalCreate>(OpenModalCreateImageContext)
    ...
    async function uploadImage() {
        ...
        if (!imageExan.name || !imageExan.src) {
            ...
            try {
                ...
                const result = await createImageExan(params.history_id, params.exan_id, ie)
        ...
        <React.Fragment>
            ...
            >
                ...
                >
                    ...
                    <Alert hidden={!nameNull} style={{ marginTop: '10px' }} severity="error">โปรดกรอกข้อมูลใหครบก่อนบันทึก!</Alert>
                        ...
                        {images.length > 0 ? (
                            <div>
                                <CardMedia
                                    ...
                                    alt="Uploaded Image"
                                />
                                <TextField error={nameNull} onChange={(e) => setImageExan({ ...imageExan, name: e.target.value.toString() })} style={{ marginTop: '10px', width: '100%' }} id="filled-basic" label="Detail" variant="filled" />
                            </div>
                        ) : (
                            <div onClick={() => document.getElementById('imageUpload')?.click()}>
                                <Paper elevation={3} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', width: '100%', height: '9rem' }}>
                    ...
                    <Typography style={{ marginTop: '10px' }} component="div">
                        <Button onClick={uploadImage} style={{ margin: '3px' }}>Save</Button>
                        <Button onClick={() => setOpen(false)} style={{ margin: '3px' }}>Cancel</Button>
                    </Typography>

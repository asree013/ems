    ...
    return (
        ...
        >
                ...
                <AccordionDetails variant='solid' sx={{ margin: '0 2px' }}>
                    <Stack className='monitor_patient_item'>
                        <FormControl orientation="horizontal" sx={{ gap: 1 }}>
                            <MonitorPatient icon={BadgeIcon} txt={patient.first_name} seconTxt={patient.last_name} />
                        </FormControl>
                        <FormControl orientation="horizontal" sx={{ gap: 1 }}>
                            <MonitorPatient icon={WcIcon} txt={'เพศ: ' + patient.gender} seconTxt={patient.age ? 'อายุ: ' + patient.age : 'อายุ: ' + 'no data'} />
                        </FormControl>
                        <FormControl orientation="horizontal" sx={{ gap: 1 }}>
                            <MonitorPatient icon={MedicalInformationIcon} txt={patient.group_blood ? 'กรุ๊ป: ' + patient.group_blood : 'กรุ๊ปเลือด: no data'} seconTxt={patient.id_card ? 'IC: ' + patient.id_card : 'IC: no data'} />
                        </FormControl>
                        <FormControl orientation="horizontal" sx={{ gap: 1 }}>
                            <MonitorPatient icon={HomeIcon} txt={patient.birthday ? 'เกิด: ' + patient.birthday : 'เกิด: no data'} seconTxt={patient.address ? 'ที่อยู่: ' + patient.address : 'ที่อยู่: no data'} />
                        </FormControl>
                    </Stack>
            ...
            <Accordion className='monitor_id_accrodion' defaultExpanded>
                ...
                <AccordionDetails variant='solid' sx={{ margin: '0 2px' }}>
                    <Stack spacing={1.5}>
                        <FormControl orientation="horizontal" sx={{ gap: 1, background: '#373535' }}>
                            <HR />
                        </FormControl>
                        <FormControl orientation="horizontal" sx={{ gap: 1 }}>
                            <Spo2 />

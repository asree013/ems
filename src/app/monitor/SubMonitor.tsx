...
import { createOrderTranfer, findAllOrderTranfer } from '@/services/order_tranfer.service'
...
export default function SubMonitor({ el_id, index }: props) {
    ...
    useEffect(() => {
        async function onFeedOrder() {
            ...
            try {
                ...
                const sot = result.filter(r => r.element_seq === el_id && r.status_order === 'Transfer')
    ...
    if (orderTranfer.includes(el_id)) {
        return (
            <>
                <ChartEcg key={index} orderTranFer={order} el_id={el_id} index={index} onChangeDeleteDeviceID={onChangeDeleteDevices} />
            </>
        )
    }
    else {
        return (
            <>
                <CreateMonitor key={index} el_id={el_id} returnMonitor={onReturnCreateMonitor} />
            </>
        )
    }
}

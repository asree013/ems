  ...
  return (
    <div>
      <div className='monitorItems'>
        <p className='create_monitor' onClick={() => setStatus(true)}
          style={{ display: 'flex', alignItems: 'center', textAlign: 'center', width: '4rem', height: '4rem', borderRadius: '4.5rem', background: 'whitesmoke', cursor: 'pointer' }}>+</p>
      </div>
      {
        status === true
          ? <div className="modal_create_monitor">
            <p style={{ fontSize: '3rem', cursor: 'pointer', color: 'white' }} onClick={() => setStatus(false)}>x</p>
            <div className="modal_item_open">
              <div style={{ display: 'flex' }}>
                <TextField id="filled-basic" label="Device id" value={txtDevice} onChange={(e) => setTxtDevice(e.target.value)} variant="filled" />
                <Button variant="contained" type='button' onClick={onClickFindDevice} style={{ background: '#1976d2' }}>Add</Button>
              </div>
              <Fab variant='extended' style={{ height: '16rem', width: '16rem' }} onClick={createMonitor}>
      ...
      }
      {
        camera === true
          ? <div className="modal_create_monitor">
            <p style={{ fontSize: '3rem', cursor: 'pointer', color: 'white' }} onClick={() => setCamera(false)}>x</p>
            <div className="modal_item_open">
              <div style={{ display: 'flex' }}>
                <TextField id="filled-basic" label="Device id" value={txtDevice} onChange={(e) => setTxtDevice(e.target.value)} variant="filled" />
                <Button variant="contained" type='button' onClick={onClickFindDevice}>Add</Button>
              </div>
              <Camera onChangeScanSuccess={getOnchange} onChangeScanFailure={getOnErorr} />
            </div>
          </div>
          : null
      }

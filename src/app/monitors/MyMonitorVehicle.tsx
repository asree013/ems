...
export default function MyMonitorVehicle({ device_id }: Props) {
  const [device, setDevice] = useState<Device>({} as Device)
  const [load, setLoad] = useState<boolean>(false)
  const findDevice = useCallback(async () => {
    setLoad(true)
    try {
      const result = await findDeviceById(device_id)
  ...
  return (
    <>
      {
        load ?
          <div className='flex flex-col items-center justify-center w-full min-h-[280px]'>
            <div role="status">
